...
import utils
from audiolib import (activitydetector, add_clipping, audioread, audiowrite,
                      is_clipped, segmental_snr_mixer)
...
def build_audio3(is_clean, params, spk_index, audio_samples_length=-1):
    ...
    for seg in range(0, num_segs):
        ...
        audio_temp = input_audio[seg *
                                 audio_samples_length: (seg + 1)*audio_samples_length]
    ...
    while file_num <= params['fileindex_end']:
        ...
        for chose_primary in chosen_clean_reverb:
            ...
            # use a randomly sampled SNR value between the specified bounds
                ...
                snr2 = np.random.randint(
                    params['snr_lower'], params['snr_upper'])
                snr3 = np.random.randint(
                    params['snr_lower'], params['snr_upper'])
            ...
            noisypath2 = os.path.join(
                params['noisyspeech_dir'], noisyfilename2)
            ...
            noisypath3 = os.path.join(
                params['noisyspeech_dir'], noisyfilename3)
...
def main_body():
    ...
    try:
        ...
        for path in Path(cfg['speech_dir2']).rglob('*.wav'):
            cleanfilenames2.append(str(path.resolve()))
    ...
    if 'noise_csv' in cfg.keys() and cfg['noise_csv'] != 'None':
        ...
        noisefilenames = glob.glob(os.path.join(
            noise_dir, params['audioformat']))
    ...
    clean_source_files, clean_clipped_files, clean_low_activity_files, \
        noise_source_files, noise_clipped_files, noise_low_activity_files = main_gen(
            params)
    ...
    total_clean = len(clean_source_files) + \
        len(clean_clipped_files) + len(clean_low_activity_files)
    total_noise = len(noise_source_files) + \
        len(noise_clipped_files) + len(noise_low_activity_files)
